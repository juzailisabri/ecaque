...
function gettable($id){
  ...
  rs_color, rs_name, rs_icon, pt2_no_kp_pemohon as enc_id, ppb_rate
  ...
  WHERE SHA2(pembayaran_ppb_id,256) = '$id'";
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    "<tr>
      ...
      <td align=\"center\">RM $ppb_rate</td>
    </tr>";
    $totalrate = $totalrate + $ppb_rate;
    $x++;
  }
...
function getppb($id){
  ...
  $s1 = "SELECT DATE_FORMAT(ppb_datetime,'%d/%m/%Y') as ppb_datetime, ppb_pngesah1_uu_id, ppb_pngesah2_uu_id, ppb_pngesah3_uu_id FROM p_payment_baucar WHERE SHA2(ppb_id,256) = '$id'";
  $result = $conn->query($s1);
  $row = $result->fetch_assoc();

  $ppb_pngesah1_uu_id = $row["ppb_pngesah1_uu_id"];
  $ppb_pngesah2_uu_id = $row["ppb_pngesah2_uu_id"];
  $ppb_pngesah3_uu_id = $row["ppb_pngesah3_uu_id"];
  $date = $row["ppb_datetime"];

  $s = "SELECT uu_id, uu_fullname, rpkt_name FROM u_user LEFT JOIN ref_pangkat ON rpkt_id = uu_rpkt_id WHERE uu_id IN ('$ppb_pngesah1_uu_id','$ppb_pngesah2_uu_id','$ppb_pngesah3_uu_id')";

  $ppb_pngesah1arr = array();
  $ppb_pngesah2arr = array();
  $ppb_pngesah3arr = array();
  $result = $conn->query($s);
  while ($row = $result->fetch_assoc()) {
    if($ppb_pngesah1_uu_id == $row["uu_id"]){ $ppb_pngesah1arr[] = $row; }
    if($ppb_pngesah2_uu_id == $row["uu_id"]){ $ppb_pngesah2arr[] = $row; }
    if($ppb_pngesah3_uu_id == $row["uu_id"]){ $ppb_pngesah3arr[] = $row; }
  }

  $ret = array (
    "p1" => $ppb_pngesah1arr[0],
    "p2" => $ppb_pngesah2arr[0],
    "p3" => $ppb_pngesah3arr[0],
    'date' => $date
  );
...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        // Title
        $this->Cell(0, 10, 'PENYATA BAYARAN HADIAH PENGAJIAN IPT', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->SetY(25);
